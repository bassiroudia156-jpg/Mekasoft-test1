...
// (5 Phase-5 canonical + 1 post-audit email-job-purge + 2 subscription-
// billing crons added 2026-08-18 + quote-expiration added 2026-08-25,
// Phase C decision #8.)
//
...
describe('vercel.json schema (CRON-07, D-20)', () => {
  ...
  it('declares schedules for the 9 canonical crons (Phase 5 + post-audit + subscriptions + quotes)', () => {
    if (!existsSync(VERCEL_JSON)) return;
    const cfg = JSON.parse(readFileSync(VERCEL_JSON, 'utf8')) as VercelConfig;
